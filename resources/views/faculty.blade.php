...
<div style="background: #fff; height: 450px; width: 100%; border-radius: 20px; margin-top:50px;">
    <div style="justify-content: space-between; display: flex; padding: 10px;">
        <h1 style="font-weight: 500; font-size: x-large; color: #545454;">Faculty</h1>
        ...
        <div class="modal-content border-none shadow-lg relative flex flex-col w-full pointer-events-auto bg-white bg-clip-padding rounded-md outline-none text-current">
          ...
          <div class="modal-body relative p-4">
            <form class="" action="{{ route('faculty.store') }}" method="POST">
                @csrf
                <div class="form-group mb-6"><input type="text" class="form-control block w-full px-3 py-1.5 text-base font-normal text-gray-700 bg-white bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none" name="faculty_code" placeholder="Faculty Code">
                  @error('faculty_code')
                          ...
                          <div class="flex justify-center">
                            <div>
                              <div class="dropdown relative">
                                ...
                                <ul class=" dropdown-menu min-w-max bg-white text-base z-50 float-left py-2 list-none text-left rounded-lg shadow-lg mt-1 hidden m-0 bg-clip-padding border-none" aria-labelledby="dropdownMenuButton2" style="position:inherit;">
                                  <li>
                                    <a class=" dropdown-item text-sm py-2 px-4 font-normal block w-full whitespace-nowrap bg-transparent text-gray-700 hover:bg-gray-100 " href="{{ route('faculty.edit', $faculties->faculty_code) }}">Update</a >
                                  </li>
                                  <li>
                                    <form action="{{ route('faculty.destroy',$faculties->faculty_code) }}" method="POST">
                                      @csrf
                                      @method('DELETE')
