        ...
        <div class="modal-content border-none shadow-lg relative flex flex-col w-full pointer-events-auto bg-white bg-clip-padding rounded-md outline-none text-current">
          ...
          <div class="modal-body relative p-4">
            <form class="" action="{{ route('department.store') }}" method="POST">
              @csrf
              <div class="form-group mb-6"><input type="text" class="form-control block w-full px-3 py-1.5 text-base font-normal text-gray-700 bg-white bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none" id="exampleInputEmail2" placeholder="Department Code" name="department_code">
              </div>
              ...
              <table class="min-w-full text-center">
                ...
                <tbody>
                  ...
                  <tr class="bg-white border-b">
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{$departments->department_code}}</td>
                        ...
                        <div>
                          <div class="dropdown relative">
                            ...
                            <ul class=" dropdown-menu min-w-max absolute bg-white text-base z-50 float-left py-2 list-none text-left rounded-lg shadow-lg mt-1 hidden m-0 bg-clip-padding border-none" aria-labelledby="dropdownMenuButton2">                            
                              <li>
                                <a class=" dropdown-item text-sm py-2 px-4 font-normal block w-full whitespace-nowrap bg-transparent text-gray-700 hover:bg-gray-100 " href="{{ route('department.edit',$departments->department_code) }}">Update</a >
                              </li>
                              <li>
                                <form action="{{ route('department.destroy',$departments->department_code) }}" method="POST">
                                  @csrf
                                  @method('DELETE')
